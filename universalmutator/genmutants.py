...
def main():
    ...
    if server:
        try:
            os.mkfifo(sourceFile)
        except:
            pass

        sourcePipe = open(sourceFile, 'rb', 0)
        print("OPENED SOURCE PIPE")
        sys.stdout.flush()
    ...
    while shouldRun:
        ...
        if server:
            currLine = bytearray("", encoding="utf-8")
            doneWithMutant = False
            msize = 0
            truncated = False
            while not doneWithMutant:
                data = sourcePipe.read(1)
                if len(data) == 0:
                    print("OTHER END CLOSED PIPE, SHUTTING DOWN MUTATION SERVER")
                    sys.exit(0)
                sys.stdout.flush()
                currLine += data
                if data == bytearray("\n", "utf-8"):
                    scurrLine = currLine.decode("utf-8", "replace")
                    #print(scurrLine)
                    if "==END OF MUTANT==" in scurrLine:
                        doneWithMutant = True
                    else:
                        if msize < 1000000:
                            source.append(scurrLine)
                            msize += len(scurrLine)
                        else:
                            truncated = True
                print("READ MUTANT FILE WITH", len(source), "LINES AND", msize, "CHARACTERS", end="")
                if truncated:
        ...
        if len(mutants) == 0:
            if server:
                with open(mdir + "fuzz.out", 'w') as mfile:
                    for line in source:
                        mfile.write(line)
            else:
        ...
        for mutant in mutants:
            ...
            if (mutantResult == "VALID") or (mutantResult == "REDUNDANT" and redundantOK):
                ...
                os.rename(tmpMutantName, mutantName)
