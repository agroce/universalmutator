...
By default the handler performs a compile check with:
  ...
  npx -y @ton/tolk-js --output-json <artifact.json> <source-file>

It runs from the source file directory so relative imports continue to work.

To override the command, set:

  UM_TOLK_CMD='npx -y @ton/tolk-js --output-json /tmp/tolk-out.json MUTANT'
...
The JSON artifact is useful for manual inspection because it contains the
generated Fift, BOC and source snapshot, but the handler itself only checks the
exit code.
"""
...
def _override_command():
    return os.environ.get("UM_TOLK_CMD") or None
...
def _default_command(target, pid):
    ...
    artifact = os.path.abspath(f".um.tolk_output.{pid}.json")
    command = subprocess.list2cmdline(
        ["npx", "-y", "@ton/tolk-js", "--output-json", artifact, os.path.basename(target)]
    )
...
def handler(tmpMutantName, mutant, sourceFile, uniqueMutants, compileFile=None):
    ...
    backupName = None
    artifact = None
    outFile = os.path.abspath(f".um.tolk_log.{pid}")
